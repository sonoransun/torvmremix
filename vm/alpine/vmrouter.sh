...
vmr_openport() {
  vmr_log "vmr_openport"
  iptables -t filter -D $host_filt_tbl -i "$1" -d "$2" -p "$3" --dport "$4" -j DROP >>"$LOG_TO" 2>&1
  iptables -t filter -I $host_filt_tbl -i "$1" -d "$2" -p "$3" --dport "$4" -j ACCEPT >>"$LOG_TO" 2>&1
}

vmr_closeport() {
  vmr_log "vmr_closeport"
  iptables -t filter -D $host_filt_tbl -i "$1" -d "$2" -p "$3" --dport "$4" -j ACCEPT >>"$LOG_TO" 2>&1
  iptables -t filter -I $host_filt_tbl -i "$1" -d "$2" -p "$3" --dport "$4" -j DROP >>"$LOG_TO" 2>&1
}

vmr_redir() {
  vmr_log "vmr_redir"
  iptables -t nat "$1" $cli_prenat_tbl -i "$2" -d "$3" -p tcp --dport "$4" -j REDIRECT --to "$5" >>"$LOG_TO" 2>&1
}
